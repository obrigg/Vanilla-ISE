        ...
        "device": {
            ...
            "connections": {
                "cli": {
                    ...
                    "settings": {
                        "GRACEFUL_DISCONNECT_WAIT_SEC": 0.5,
                        "POST_DISCONNECT_WAIT_SEC": 0.5,
                    },
                }
            },
            "credentials": {
...
def get_all_NADs():
    ...
    try:
        ...
        complete_NAD_list = []
        while not isDone:
            NAD_list = requests.get(
                url=url, headers=headers, auth=auth, verify=False
            ).json()
...
def update_ise_endpoint_group(mac_address: str, group_name: str):
    ...
    if ise_group_id != "ERROR":
        ...
        if response.status_code == 200:
            ...
            data = (
                '{"ERSEndPoint": {"groupId": "%s","staticGroupAssignment": "true"}}'
                % ise_group_id
            )
            ...
            response = requests.post(
                url=url, data=json.dumps(data), auth=auth, headers=headers, verify=False
            )
...
def initialize_ise(name, passw):
    ...
    try:
        Iresponse = requests.get(
            url=url, headers=headers, auth=user_auth, verify=False, timeout=5
        )
    ...
    if mac != "ERROR":
        ...
        if response.status_code != 200:
            ...
            if response.text.rfind("authStatusElements") == -1:
                ...
                status_details["MACAddress"] = status["authStatusOutputList"][
                    "authStatusList"
                ]["@key"]
                status_details["NAD"] = status["authStatusOutputList"][
                    "authStatusList"
                ]["authStatusElements"]["network_device_name"]
                status_details["Interface"] = status["authStatusOutputList"][
                    "authStatusList"
                ]["authStatusElements"]["nas_port_id"]
                status_details["AuthMethod"] = status["authStatusOutputList"][
                    "authStatusList"
                ]["authStatusElements"]["authentication_method"]
                ...
                if (
                    ...
                    status_details["FailureReason"] = status["authStatusOutputList"][
                        "authStatusList"
                    ]["authStatusElements"]["failure_reason"]
                # Add SGT, if exists
                if (
                    "cts_security_group"
                    ...
                    status_details["SGT"] = status["authStatusOutputList"][
                        "authStatusList"
                    ]["authStatusElements"]["cts_security_group"]
                else:
        ...
        for client in auth_sessions["interfaces"][interface]["client"]:
            if (auth_sessions["interfaces"][interface]["client"][client]["domain"]!= "UNKNOWN"):
                ...
                try:
                    if (
                        "local_policies"
                        in auth_details["interfaces"][interface]["mac_address"][client]
                    ):
                        session["Vlan"] = auth_details["interfaces"][interface][
                            "mac_address"
                        ][client]["local_policies"]["vlan_group"]["vlan"]
                    if (
                        "server_policies"
                        in auth_details["interfaces"][interface]["mac_address"][client]
                    ):
                        server_policies = auth_details["interfaces"][interface][
                            "mac_address"
                        ][client]["server_policies"]
...
def read_voucher_list():
    ...
    print(
        f"Voucher list content (Current Epoch time: {int(time())}):\
        \n======================="
    )
...
def add_host_voucher(mac_address: str, duration: int, voucher_group: str, user: str):
    ...
    if mac != "ERROR":
        try:
            ...
            for voucher in voucher_list:
                if voucher["type"] == "host" and voucher["mac"] == mac:
                    pp(
                        f"[red]ERROR: MAC {mac} already has a voucher. Kindly revoke it first."
                    )
        ...
        try:
            ...
            send_syslog(
                f"Voucher created! MAC: {mac}, duration: {duration} hours, voucher group: {voucher_group}, user: {user}"
            )
            return "Done"
        except:
...
def add_port_voucher(device_ip: str, interface: str, duration: int, user: str):
    ...
    try:
        ...
        for voucher in voucher_list:
            if voucher["type"] == "port":
                if (
                    voucher["switch_ip"] == device_ip
                    and voucher["interface"] == interface
                ):
                    pp(
                        f"[red]ERROR: {interface} on {device_ip} already has a voucher. Kindly revoke it first."
                    )
    ...
    try:
        ...
        pp(
            f"[red]ERROR: Problem removing the command {command} from {interface} on {device_ip}."
        )
        device.disconnect()
    ...
    try:
        print(
            f"Adding {interface} on switch {device_ip} with a duration of {duration} hours, user: {user}"
        )
        voucher = {
            "type": "port",
            "switch_ip": device_ip,
        ...
        pp(
            f"[red]ERROR: Wasn't able to update the voucher file - reverting port configuration"
        )
        device.configure(f"interface {interface} \n {command}")
        device.disconnect()
...
def revoke_port_voucher(device_ip: str, interface: str, command: str, user: str):
    ...
    try:
        ...
        pp(
            f"[red]ERROR: Problem restoring the command {command} from {interface} on {device_ip}."
        )
        device.disconnect()
    ...
    try:
        ...
        return "ERROR: Wasn't able to update the voucher file"
    send_syslog(
        f"Voucher revoked! Switch: {device_ip}, interface: {interface}, user: {user}"
    )
    device.disconnect()
    return "Done"
...
def send_syslog(data: str):
    ...
    if syslog_server != "":
        print(
            f"Sending syslog server {syslog_server} the following message: \
            \n{data}"
        )
